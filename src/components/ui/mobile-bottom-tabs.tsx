...
export function EmployeeMobileBottomTabs({
  ...
  return (
    <nav
      aria-label={label}
      className={cn(
        "border-outline-variant bg-surface-container-lowest shadow-level-2 fixed right-0 bottom-0 left-0 z-40 border-t px-2 py-2 md:hidden",
        className,
      )}
    >
      <div className="grid grid-cols-4 gap-1">
        {items.map((item) => {
          ...
          const itemClassName = cn(
            "inline-flex min-h-12 flex-col items-center justify-center gap-1 rounded px-2 text-xs font-semibold text-on-surface-variant transition",
            item.active
